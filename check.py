...
"""
    given coordinates
    if k and q have the same col, 
    or the same row, return true
    if dif btwn k col and q col == dif btwn kr and qr:
        return true
        dif btwn letters?
...
def check(king, queen):
    ...
    if abs(ord(king[0]) - ord(queen[0])) == abs(int(king[1]) - int(queen[1])):
        return True
